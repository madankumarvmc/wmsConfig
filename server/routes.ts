...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/task-planning/by-group/:inventoryGroupId", async (req, res) => {
    try {
      const inventoryGroupId = parseInt(req.params.inventoryGroupId);
      const configuration = await storage.getTaskPlanningConfigurationByGroup(inventoryGroupId);
      res.json(configuration || null);
    } catch (error) {
      res.status(500).json({ error: "Failed to fetch task planning configuration for group" });
    }
  });
  ...
  app.get("/api/task-execution/by-group/:inventoryGroupId", async (req, res) => {
    try {
      const inventoryGroupId = parseInt(req.params.inventoryGroupId);
      const configuration = await storage.getTaskExecutionConfigurationByGroup(inventoryGroupId);
      res.json(configuration || null);
    } catch (error) {
      res.status(500).json({ error: "Failed to fetch task execution configuration for group" });
    }
  });
